...
def test_normalizes_exercise_alias_for_items_and_reports(tmp_path: Path) -> None:
    seed_path = tmp_path / "schedule_seed.sql"
    db_path = tmp_path / "schedule.db"
    seed_path.write_text(
        "\n".join(
            [
                ...
                "  id, user_id, activity_type, title, instructions_json, window_start_local, window_end_local, display_order, active, created_at, updated_at",
                ") VALUES (",
                "  'sched_walk', 'patient-1', 'exercise', 'Walk', '[\"10 mins\"]', '13:00', '14:00', 1, 1, '2026-02-20T10:00:00Z', '2026-02-20T10:00:00Z'",
                ");",
            ]
        ),
        encoding="utf-8",
    ...
    assert items[0].activity_type.value == "activity"
    assert saved.activity_type.value == "activity"
    assert reports[0].activity_type.value == "activity"
