...
def _write_structured_allergy_seed(seed_path: Path) -> None:
    seed_path.write_text(
        "\n".join(
            [
                "CREATE TABLE IF NOT EXISTS patient_profiles (",
                ...
                "  contraindications_json, family_history_json, biomarker_targets_json, notes, updated_at",
                ") VALUES (",
                "  'patient-structured', 'Demo Structured', 49, 'male',",
                '  \'[{"name":"Hypertension","status":"active"}]\',',
                '  \'[{"name":"Amlodipine","status":"ongoing"}]\',',
                '  \'[{"allergen":"Penicillin","reaction":"Rash","severity":"mild"}, "Dust"]\',',
                "  '[\"Avoid NSAIDs\"]',",
                "  '[\"Family history of hypertension\"]',",
                '  \'[{"biomarker":"Blood Pressure","target":"<130/80","unit":"mmHg"}]\',',
                "  'Structured allergy note',",
                "  '2026-02-20T10:00:00Z'",
                ");",
            ]
        ),
...
def test_get_by_user_id_supports_structured_allergies(tmp_path: Path) -> None:
    seed_path = tmp_path / "seed.sql"
    db_path = tmp_path / "profiles.db"
    _write_structured_allergy_seed(seed_path)
    ...
    profile = repo.get_by_user_id("patient-structured")

    assert profile is not None
    assert isinstance(profile.allergies[0], AllergyRecord)
    assert profile.allergies[0].allergen == "Penicillin"
    assert profile.allergies[0].reaction == "Rash"
    assert profile.allergies[1] == "Dust"
