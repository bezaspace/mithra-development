...
class _DashboardRepositoryStub:
    def get_adherence_stats(self, user_id):
        return {"overall": 85, "weekly": [80, 85, 90, 88], "by_activity": {"medication": 95, "diet": 80}}
...
def test_schedule_tools_emit_expected_payloads() -> None:
    tools = build_schedule_tools(_ScheduleServiceStub(), _DashboardRepositoryStub())
    assert len(tools) == 4
    ...
    assert stats["overallAdherence"] == 85
    assert stats["weeklyAdherence"] == [80, 85, 90, 88]
    assert stats["activityBreakdown"] == {"medication": 95, "diet": 80}
