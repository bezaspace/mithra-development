...
def build_instruction(profile_summary: str | None = None) -> str:
    ...
    base_instruction = (
        ...
        "VISUAL RENDERING RULES (CRITICAL): "
        "Whenever the user asks about any metric, progress indicator, or schedule item, you MUST invoke the matching tool BEFORE producing any spoken response. "
        "Each tool renders a dedicated UI on the user's screen, and only ONE UI is visible at a time — the most recent tool result replaces the previous one. "
        "Therefore, call exactly ONE tool per topic. Do not call multiple metric tools in the same turn just to 'show more'. If the user asks about a different metric next, call only that tool. "
        "Tool-to-topic mapping (use the most specific tool; never substitute): "
        "(1) 'what should I do now', 'next activity', 'current task', or 'what's scheduled' -> call get_current_schedule_item. "
        "(2) 'my full schedule', 'today's plan', 'list everything today' -> call get_today_schedule. "
        "(3) 'adherence', 'compliance', 'how well am I following the plan', 'weekly adherence', 'completion rate' -> call get_adherence_score. "
        "(4) 'physiotherapy score', 'physio progress', 'recovery score', 'how is my physiotherapy trending' -> call get_physiotherapy_score. "
        "(5) 'pain', 'pain index', 'pain level', 'is my pain improving' -> call get_pain_index. "
        "Never call get_adherence_score when the user asks specifically about physiotherapy or pain, and never call get_physiotherapy_score or get_pain_index when the user asks about adherence. "
