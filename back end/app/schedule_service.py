...
class ScheduleService:
    ...
    def get_today_schedule(
        ...
        *,
        ...
        ctx = self._build_time_context(
            timezone_name=timezone_name, reported_at_iso=None, date_str=date_str
        )
            ...
            item_cards.append(
                {
                    ...
                    "latestReport": self._serialize_report_detail(latest)
                    if latest
                    else None,
                }
            )
    ...
    def get_current_schedule_item(
        ...
        *,
        ...
        ctx = self._build_time_context(
            timezone_name=timezone_name, reported_at_iso=now_iso
        )
        local_now_time = ctx.local_now.time().replace(second=0, microsecond=0)
        items = self._repository.list_active_items(user_id)

        in_window_items = [
            item for item in items if self._is_time_in_window(local_now_time, item)
        ]
        if in_window_items:
            chosen = sorted(
                in_window_items,
                key=lambda item: (item.display_order, item.window_start_local, item.id),
            )[0]
    ...
    def save_adherence_report(
        ...
        *,
        ...
        context = self._build_time_context(
            timezone_name=timezone_name, reported_at_iso=reported_at_iso
        )
    ...
    @staticmethod
    def _next_upcoming_item(
        local_now: time, items: list[ScheduleItem]
    ) -> ScheduleItem | None:
        upcoming = [
            item
            for item in items
            if time.fromisoformat(item.window_start_local) > local_now
        ]
        if not upcoming:
            return None
        return sorted(
            upcoming,
            key=lambda item: (item.window_start_local, item.display_order, item.id),
        )[0]

    @staticmethod
    def _latest_report_by_item(
        reports: list[AdherenceReport],
    ) -> dict[str, AdherenceReport]:
    ...
    def _build_summary(
        *,
        ...
        parts.append(
            "Followed plan." if followed_plan else "Did not fully follow plan."
        )
        if felt_after:
    ...
    def _build_saved_payload(
        ...
        *,
        ...
        if (
            resolved_schedule_item_id
            and resolved_schedule_item_id != report.schedule_item_id
        ):
    ...
    def _resolve_schedule_item(
        ...
        *,
        ...
        substring_matches = [
            ...
            for item in items
            if normalized_query
            and normalized_query in self._normalize_label(item.title)
        ]
        ...
        type_matches = [
            item for item in items if normalized_query == item.activity_type.value
        ]
        if type_matches:
    ...
    def _infer_activity_type_from_alias(label: str) -> ScheduleActivityType | None:
        alias_map: list[tuple[ScheduleActivityType, set[str]]] = [
            (
                ScheduleActivityType.DIET,
                {"meal", "breakfast", "lunch", "dinner", "snack", "food"},
            ),
            (
                ScheduleActivityType.MEDICATION,
                {"medication", "medicine", "meds", "pill", "tablet", "dose"},
            ),
            (
                ScheduleActivityType.ACTIVITY,
                {"activity", "exercise", "walk", "workout", "run", "yoga"},
            ),
    ...
    def _choose_best_item(
        items: list[ScheduleItem], context: ScheduleTimeContext
    ) -> ScheduleItem:
        now_local = context.local_now.time().replace(second=0, microsecond=0)
        in_window = [
            item
            for item in items
            if ScheduleService._is_time_in_window(now_local, item)
        ]
        if in_window:
            return sorted(
                in_window,
        ...
        return sorted(
            ...
            key=lambda item: (
                abs(
                    (
                        time.fromisoformat(item.window_start_local).hour * 60
                        + time.fromisoformat(item.window_start_local).minute
                    )
        ...
        *,
        ...
        if date_str:
            ...
            local_now = datetime.combine(
                parsed_date, time(hour=12, minute=0), tzinfo=zone
            )
